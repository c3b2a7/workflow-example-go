...
PROJECT_NAME=workflow-example-go
OWNER=c3b2a7
REPO="${PROJECT_NAME}"
...
# GitHub artifact attestation verification option.
VERIFY_ATTESTATION=false
# this is the earliest tag in the repo where install.sh script was available
INSTALL_SH_SUPPORTED_VERSION=v0.0.0
# this is the earliest tag in the repo where artifact attestations were introduced in the release process
VERIFY_ATTESTATION_SUPPORTED_VERSION=v0.0.0
# this is the earliest tag in the repo where the -v flag was introduced to this install.sh script
VERIFY_ATTESTATION_FLAG_VERSION=v0.0.0
...
download_github_release_checksums() (
  download_github_release_checksums_files "$@" "sha256_checksums.txt"
)
...
# download_and_install_asset [release-url-prefix] [download-path] [install-path] [name] [os] [arch] [tag] [format] [binary]
#
...
download_and_install_asset() (
  ...
  if ! asset_filepath=$(download_asset "${download_url}" "${download_path}" "${name}" "${os}" "${arch}" "${tag}" "${format}"); then
...
# verify_attestation [artifact-path]
#
# verifies the GitHub artifact attestation for a downloaded release artifact.
#
verify_attestation() {
  artifact_path=$1

  log_trace "verifying artifact attestation for ${artifact_path}"

  log_file=$(mktemp)

  gh attestation verify "${artifact_path}" -R "${OWNER}/${REPO}" >"${log_file}" 2>&1
...
# download_asset [release-url-prefix] [download-path] [name] [os] [arch] [tag] [format]
#
...
download_asset() (
  ...
  log_trace "download_asset(url=${download_url}, destination=${destination}, name=${name}, os=${os}, arch=${arch}, tag=${tag}, format=${format})"
  ...
  if ! hash_sha256_verify "${asset_filepath}" "${checksums_filepath}"; then
    log_err "checksums verification failed"
    return 1
  fi
  log_info "checksums verification succeeded"

  if [ "$VERIFY_ATTESTATION" = true ]; then
    if ! verify_attestation "${asset_filepath}"; then
      log_err "attestation verification failed"
      return 1
    fi
    log_info "attestation verification succeeded"
  fi
...
prep_github_verification() {
  ...
  if ! compare_semver "$version" "$VERIFY_ATTESTATION_SUPPORTED_VERSION"; then
    log_err "${PROJECT_NAME} release '$version' does not support artifact attestation verification"
    log_err "you can still install ${PROJECT_NAME} by removing the -v flag or using a release that supports artifact attestations (>= '$VERIFY_ATTESTATION_SUPPORTED_VERSION')"
    log_err "aborting installation"
    return 1
  else
    log_trace "${PROJECT_NAME} release '$version' supports artifact attestation verification (>= '$VERIFY_ATTESTATION_SUPPORTED_VERSION')"
  fi

  # will invoking an earlier version of this script work (considering the -v flag)?
  if ! compare_semver "$version" "$VERIFY_ATTESTATION_FLAG_VERSION"; then
    # the -v argument did not always exist, so we cannot be guaranteed that invoking an earlier version of this script
    # will work (error with "illegal option -v"). However, the user requested attestation verification, so we will
    # attempt to install the application with this version of the script (keeping attestation verification).
    DOWNLOAD_TAG_INSTALL_SCRIPT=false
    log_debug "provided version install script does not support -v flag (>= '$VERIFY_ATTESTATION_FLAG_VERSION'), using current script for installation"
  else
    log_trace "provided version install script supports -v flag (>= '$VERIFY_ATTESTATION_FLAG_VERSION')"
  fi

  # check to see if the gh binary is installed
  if is_command gh; then
    log_trace "gh binary is installed"
  else
    log_err "artifact attestation verification is requested but gh is not installed (see https://cli.github.com to install it)"
    return 1
  fi
}

prep_run_install_script() {
  version="$1"
  if ! compare_semver "$version" "$INSTALL_SH_SUPPORTED_VERSION"; then
    log_err "${PROJECT_NAME} release '$version' does not support using install.sh to install it"
    log_err "you can still install ${PROJECT_NAME} by removing specified version or using a release that supports (>= '$INSTALL_SH_SUPPORTED_VERSION')"
    log_err "aborting installation"
    return 1
  fi
}
...
Usage: $0 [-v] [-b DIR] [-d] [TAG]
  ...
  -v      verify GitHub artifact attestation (require gh binary to be installed).
...
EOF
  ...
  if ! prep_run_install_script "$version"; then
    return 1
  fi
  if ! prep_github_verification "$version"; then
    return 1
  fi
  ...
  if ! download_and_install_asset "${download_url}" "${download_dir}" "${install_dir}" "${PROJECT_NAME}" "${os}" "${arch}" "${tag}" "${format}" "${binary}"; then
